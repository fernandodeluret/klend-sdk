...
import { WSOL_MINT, getLocalSwapper, getPriceAinB } from './runner/leverage_utils';
import { setupStrategyAndMarketWithInitialLiquidity, newUser, createLookupTable } from './runner/setup_utils';
import Decimal from 'decimal.js';
import { assert } from 'chai';
import { Price } from './runner/kamino/price';
...
import { getMint } from './runner/token_utils';
...
describe('Leverage SDK kTokens swapper tests', function () {
  it('swap_A_to_kAB_A_is_SOL_B_is_SPL', async function () {
    ...
    const tx = await buildVersionedTransaction(env.connection, depositor.publicKey, swapIxns, [
    ...
    await sendAndConfirmVersionedTransaction(env.connection, tx, 'processed', { skipPreflight: true });
    ...
    const kTokenDecimals = (await getMint(env.connection, ktokenMint)).decimals;
  ...
  it('swap_A_to_kAB_A_is_SPL_B_is_SOL', async function () {
    ...
    const depositDecimals = (await getMint(env.connection, tokenAMint)).decimals;
  ...
  it('swap_B_to_kAB_A_is_SPL_B_is_SPL', async function () {
    ...
    const depositDecimals = (await getMint(env.connection, tokenBMint)).decimals;
  ...
  it('swap_kAB_to_A_A_is_SOL_B_is_SPL', async function () {
    ...
    const tokenADecimals = (await getMint(env.connection, tokenAMint)).decimals;
  ...
  it('swap_kAB_to_B_A_is_SOL_B_is_SPL', async function () {
    ...
    const tokenBDecimals = (await getMint(env.connection, tokenBMint)).decimals;
